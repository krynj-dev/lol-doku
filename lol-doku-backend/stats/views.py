...
class StatsAPIView(APIView):
    ...
    def post(self, request: Request):
        ...
        x_rule = todays_puzzle.puzzle.assoc_rules.get(
            index=x, axis=PuzzleRule.RuleAxis.X
        ).rule
        y_rule = todays_puzzle.puzzle.assoc_rules.get(
            index=y, axis=PuzzleRule.RuleAxis.Y
        ).rule
        # Grab all the objects
        guesses = CorrectPlayerGuess.objects.filter(
            roster=todays_puzzle, x=x_rule, y=y_rule
        )
        serializer = CorrectPlayerGuessSerializer(
            guesses, many=True, context={"request": request}
        )
        data_list = [d for d in serializer.data]
        total_games = len(
            Game.objects.all().filter(
                rostered_puzzle=todays_puzzle, status=Game.Status.FINALISED
            )
        )
        return Response(
            {
