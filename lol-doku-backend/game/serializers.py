...
class GameRosterSerializer(serializers.ModelSerializer):
    puzzle = serializers.HyperlinkedRelatedField(
        queryset=Puzzle.objects.all(), view_name="puzzle-detail"
    )
    ...
    def create(self, validated_data):
        ...
        x_rule = [
            r.rule
            for r in puzzle.assoc_rules.filter(axis=PuzzleRule.RuleAxis.X).order_by(
                "index"
            )
        ]
        y_rule = [
            r.rule
            for r in puzzle.assoc_rules.filter(axis=PuzzleRule.RuleAxis.Y).order_by(
                "index"
            )
        ]
        for x in range(len(x_rule)):
            for y in range(len(y_rule)):
                valid_players = set(x_rule[x].valid_players.all()) & set(
                    y_rule[y].valid_players.all()
                )
                for plr in valid_players:
                    cpg = CorrectPlayerGuess.objects.create(
                        roster=instance, x=x_rule[x], y=y_rule[y], player=plr
                    )
                    cpg.save()
        return instance
...
class GameGuessSerializer(serializers.ModelSerializer):
    player = serializers.SlugRelatedField(
        queryset=Puzzle.objects.all(), slug_field="display_name"
    )
...
class GameSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Game
        fields = ["remaining_guesses", "puzzle", "guesses", "status"]
