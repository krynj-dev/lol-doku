...
class PuzzleViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request: Request):
        ...
        try:
            ...
            p.delete()
            return Response(
                json.dumps({"status": 404, "message": f"Rule {r_key} not found"}),
                status=404,
            )
        except Exception as e:
            p.delete()
            return Response(
                json.dumps({"status": 500, "message": "Server error", "error": str(e)}),
                status=500,
            )
        if not is_solvable(xes[0], xes[1:], yes):
            return Response(
                json.dumps({"status": 400, "message": "Invalid puzzle"}), status=400
            )
        elif puzzle_exists(xes, yes):
            return Response(
                json.dumps({"status": 400, "message": "Puzzle exists"}), status=400
            )
        else:
            p.save()
...
class PuzzleGenerateApiView(APIView):
    ...
    def post(self, request: Request):
        ...
        p = create_puzzle(
            min_answers, allowed_regions, rule_type_minimums, included_rules
        )
        if p is None:
    ...
    for db_puzzle in db_puzzles:
        db_axis_one = set(
            [
                ...
                for r in [
                    pr.rule
                    for pr in db_puzzle.assoc_rules.filter(axis=PuzzleRule.RuleAxis.X)
                ]
            ]
        )
        db_axis_two = set(
            [
                ...
                for r in [
                    pr.rule
                    for pr in db_puzzle.assoc_rules.filter(axis=PuzzleRule.RuleAxis.Y)
                ]
            ]
        )
        if (xes == db_axis_one and yes == db_axis_two) or (
            xes == db_axis_two and yes == db_axis_one
        ):
            return True
    return False
