...
def is_valid_guess(rule_1: Rule, rule_2: Rule, player: Player):
    options = set(rule_1.valid_players.primary.all()).intersection(set(rule_2.valid_players.primary.all()))
...
def rate_difficulty(columns: set, rows: set):
    ...
    for c in column_rules:
        for r in row_rules:
            cvp = set([x.display_name for x in c.valid_players.primary.all()])
            rvp = set([x.display_name for x in r.valid_players.primary.all()])
...
def is_solvable(candidate: str, into_set: list, cross_set: list, include_secondary: bool):
    perfect_players = set()
    cand_set = into_set + [candidate]
    for c in cand_set:
        for r in cross_set:
            options = get_links(c, r, include_secondary).difference(perfect_players)
            if len(options) == 0: # Change to 0 for 1+ answers okay
                return False
            elif len(options) == 1:
                perfect_players.add(list(options)[0])
    return True
...
def get_links(key_a, key_b, include_secondary: bool):
    r1_prim = set([x.display_name for x in key_a.valid_players.primary.all()])
    r2_prim = set([x.display_name for x in key_b.valid_players.primary.all()])
    common_players = r1_prim.intersection(r2_prim)
    if include_secondary:
        r1_sec = set([x.display_name for x in TieredValidPlayers.objects.get(valid_playerz=key_a[0]).secondary.all()])
        r2_sec = set([x.display_name for x in TieredValidPlayers.objects.get(valid_playerz=key_b[0]).secondary.all()])
        common_players |= r1_prim.intersection(r2_sec)
        common_players |= r1_sec.intersection(r2_prim)
        common_players |= r1_sec.intersection(r2_sec)
    # print(common_players)
    return common_players

def restrict_non_team_rules(key_set: set, into_set: list, cross_set: list):
    new_keys = key_set
    # Ensure at least 3 teams in every puzzle
    min_teams = 3
...
def restrict_teammate_team_cross(key_set: set, into_set: list, cross_set: list):
    ...
    for r in into_set:
        if r.rule_type == "team":
            new_keys = set(filter(lambda x: x.key not in [p.display_name for p in r.valid_players.primary.all()], new_keys))
        elif r.rule_type == "teammate":
            team_membership = [x.valid_playerz.get().key for x in Player.objects.get(display_name=r.key).primary_valid_players.all() if x.valid_playerz.get().rule_type == "team"]
...
def get_valid_options(rule_set, into_set: list, cross_set: list, exclusions: list, include_secondary: bool):
    ...
    for cross_key in cross_set:
        valid_crosses = ([y.key for y in TieredValidCrosses.objects.get(rule=cross_key).primary.all()],
            [y.key for y in TieredValidCrosses.objects.get(rule=cross_key).primary.all()] + [y.key for y in TieredValidCrosses.objects.get(rule=cross_key).secondary.all()]
            )[include_secondary]
    ...
    for j in range(len(key_list)):
        if is_solvable(key_list[j], into_set, cross_set, include_secondary):
            candidate_i = j
            break
    if candidate_i == -1:
        return None
...
def create_puzzle(include_secondary: bool):
    ...
    rule_set = [x for x in (Rule.objects.annotate(num_crosses=Count('primary_valid_crosses')).filter(num_crosses__gte=10))]
    ...
    while len(rows) < 3 or len(columns) < 3:
        ...
        if (i == 0 and len(columns) < 3) or (i == 1 and len(rows) == 3):
            cc = get_valid_options(rule_set, columns, rows, column_exclude, include_secondary)
            if cc is not None:
                columns.append(cc)
                i = 1
            elif len(rows) > 0:
            ...
            cc = get_valid_options(rule_set, rows, columns, row_exclude, include_secondary)
            if cc is not None:
                rows.append(cc)
                i = 0
            elif len(columns) > 0:
