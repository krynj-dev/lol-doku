...
class ValidCrossesSerializer(serializers.ModelSerializer):
    crosses = serializers.SlugRelatedField(
        many=True, slug_field="key", queryset=Rule.objects.all()
    )
...
class RuleSerializer(serializers.ModelSerializer):
    ...
    valid_players = serializers.SlugRelatedField(
        many=True, slug_field="display_name", queryset=Player.objects.all()
    )
