...
export function generate_puzzle(
	restrictionFns: ((a: PuzzleRule[], b: PuzzleRule[], candidate: PuzzleRule) => boolean)[]
): Puzzle {
	...
	while (columns.length < 3 || rows.length < 3) {
		...
		if ((cr_flag && columns.length < 3) || rows.length == 3) {
			added = selectRule(columns, rows, perfect_players, restrictionFns);
			if (added && !removed_columns.find((e) => e == added.op)) {
				columns.push(added);
				// console.log("added col ", added.op)
				cr_flag = !cr_flag;
				removed_columns = [];
			} else {
				if (rows.length > 0) {
			...
			added = selectRule(rows, columns, perfect_players, restrictionFns);
			if (added && !removed_rows.find((e) => e == added.op)) {
				rows.push(added);
				// console.log("added row ", added.op)
				cr_flag = !cr_flag;
				removed_rows = [];
			} else {
				if (columns.length > 0) {
...
function selectRule(
	...
	let tried = new Set();
	let succeeded = false;
	let filtered_list = Object.keys(teams)
		.filter(key => restrictionFns.every(fn => fn(a, b, teams[key] as PuzzleRule)))
		.reduce((res: { [key: string]: Team; }, key) => (res[key] = teams[key], res), {});
	// console.log(Object.keys(teams).length, Object.keys(filtered_list).length);
	let filtered_keys = Object.keys(filtered_list);
	if (filtered_keys.length === 0) return undefined;
	do {
		team_to_add = filtered_list[filtered_keys[Math.floor(Math.random() * filtered_keys.length)]] as PuzzleRule;
		// check if already picked
		if (
			a.find((e) => team_to_add.op == e.op) ||
			b.find((e) => team_to_add.op == e.op) ||
			team_to_add.highest_level != 'Primary') {
		...
		// let restriction_check: boolean[] = [];
		// restrictionFns.forEach(fn => {
		// 	restriction_check.push(fn(a, b, team_to_add));
		// })
		// if (restriction_check.some(e => e === false)) {
		// 	tried.add(team_to_add.op);
		// 	continue;
		// }
		succeeded = true;
	} while (!succeeded && tried.size < filtered_keys.length);
	if (succeeded) {
		return team_to_add;
	} else {
