...
export function generate_puzzle(): Puzzle | null {
	...
	do {
		...
		teams.forEach((team) => {
			let intersect = columns[0].players.filter(
				(value) => team.players.includes(value) && columns[0] != team
			);
	...
	teams.forEach((team) => {
		...
		for (let i = 0; i < rows.length; i++) {
			intersect = rows[i].players.filter(
				(value) =>
					team.players.filter((p) => !perfect_players.includes(p)).includes(value) &&
					rows[i] != team &&
					columns[0] != team
			);
			if (intersect.length < min_overlap) {
