...
_parse_map_player = {"age": parse_int, "active": parse_bool}
...
COMMIT;"""
    ...
    for k, v in added_players.items():
        insertions.append(
            insertion_sql.format(
                parse_varchar(k),
                parse_varchar(v["name"], True),
                parse_int(v["age"]),
                parse_varchar(v["country"], True),
                parse_varchar(v["residency"], True),
            )
        )
    ...
    for k, v in updated_players.items():
        updating_fields = {
            (a, "real_name")[a == "name"]: b
            for a, b in v.items()
            if a != "alternate_names"
        }
        if len(updating_fields) > 0:
            ...
            set_sql = [
                "{field}={value}".format(field=k, value=get_parse_func(k)(v))
                for k, v in updating_fields.items()
            ]
            unset_sql = [
                "{field}={value}".format(
                    field=a,
                    value=get_parse_func(a)(
                        old_players[k][(a, "name")[a == "real_name"]]
                    ),
                )
                for a, _ in updating_fields.items()
            ]
            updates.append(
                update_sql.format(changes=", ".join(set_sql), old_key=parse_varchar(k))
            )
            downdates.append(
                update_sql.format(
                    changes=", ".join(unset_sql),
                    old_key=parse_varchar(
                        k if "display_name" not in v else v["display_name"]
                    ),
                )
            )
        if "alternate_names" in v:
            alt_names[k if "display_name" not in v else v["display_name"]] = set(
                v["alternate_names"]
            )
    sql_updates = "\n".join(updates)
    sql_downdates = "\n".join(downdates)
    ...
    for k, v in alt_names.items():
        for n in v:
            new_names.append(
                name_insertion_sql.format(
                    alt_name=parse_varchar(n), main_key=parse_varchar(k)
                )
            )
            del_names.append(
                name_deletion_sql.format(
                    alt_name=parse_varchar(n), main_key=parse_varchar(k)
                )
            )
    sql_alt_names = ",\n".join(new_names)
    ...
    return (
        migrate_sql.format(
            ...
            sql_alt_names=sql_alt_names,
            date=parse_varchar(datetime.now().strftime("%Y-%m-%d")),
        ),
        restore_sql.format(
            sql_removes=sql_removes,
            sql_downdates=sql_downdates,
            sql_alt_names=sql_del_names,
        ),
    )
...
COMMIT;"""
    ...
    for k, v in added_teams.items():
        insertions.append(
            insertion_sql.format(
                parse_varchar(v["becomes"]),
                parse_varchar(v["came_from"]),
                parse_varchar(v["highest_level"], True),
                parse_varchar(v["image"]),
                parse_varchar(v["name"]),
                parse_varchar(k),
                parse_varchar(v["region"]),
                parse_varchar(v["short"]),
            )
        )
    ...
    for k, v in updated_teams.items():
        ...
        updating_fields = {
            a: b
            for a, b in v.items()
            if a not in ["other_names", "sister_teams", "active"]
        }
        if len(updating_fields) > 0:
            set_sql = [
                "{field}={value}".format(field=a, value=get_parse_func(a)(b))
                for a, b in updating_fields.items()
            ]
            unset_sql = [
                "{field}={value}".format(
                    field=a, value=get_parse_func(a)(old_teams[k][a])
                )
                for a, _ in updating_fields.items()
            ]
            updates.append(
    ...
    for k, v in alt_names.items():
        ...
        if len(v) > 0:
            del_names.append(
                name_deletion_sql.format(
                    main_key=parse_varchar(k),
                    alt_names=", ".join([parse_varchar(n) for n in v]),
    ...
    return (
        migrate_sql.format(
            ...
            sql_alt_names=sql_alt_names,
            date=parse_varchar(datetime.now().strftime("%Y-%m-%d")),
            sql_sister_teams=sql_sister_teams,
            sql_deactivate=removed_teams,
            sql_reactivate=readded_teams,
        ),
        restore_sql.format(
            sql_removes=sql_removes,
            sql_downdates=sql_downdates,
            sql_alt_names=sql_del_names,
            sql_del_sisters=sql_del_sisters,
            sql_reactivate=removed_teams,
            sql_deactivate=readded_teams,
        ),
    )
...
COMMIT;"""
    ...
    for k, v in added_rules.items():
        insertions.append(
            insertion_sql.format(parse_varchar(k), parse_varchar(v["type"]))
        )
    ...
    for k, v in updated_rules.items():
        ...
        updating_fields = {
            a: b
            for a, b in v.items()
            if a not in ["exclusive_crosses", "valid_players", "active"]
        }
        if len(updating_fields) > 0:
            set_sql = [
                "{field}={value}".format(field=k, value=get_parse_func(k)(v))
            ...
            unset_sql = [
                "{field}={value}".format(
                    field=a, value=get_parse_func(a)(old_rules[k][a])
                )
                for a, _ in updating_fields.items()
            ]
            updates.append(
    ...
    for k, v in valid_players.items():
        for n in v:
            new_players.append(
                player_insertion_sql.format(
                    player_name=parse_varchar(n), rule_key=parse_varchar(k)
                )
            )
            del_players.append(
                player_deletion_sql.format(
                    player_name=parse_varchar(n), rule_key=parse_varchar(k)
                )
            )
    sql_alt_names = ",\n".join(new_players)
    ...
    return (
        migrate_sql.format(
            ...
            sql_alt_names=sql_alt_names,
            date=parse_varchar(datetime.now().strftime("%Y-%m-%d")),
            sql_deactivate=removed_rules,
            sql_reactivate=readd_rules,
        ),
        restore_sql.format(
            sql_removes=sql_removes,
            sql_downdates=sql_downdates,
            sql_alt_names=sql_del_names,
            sql_del_players=sql_del_names,
            sql_reactivate=removed_rules,
            sql_deactivate=readd_rules,
        ),
    )
...
def generate_update_sql():
    ...
    with open(
        f"data/{time_path}/rules/worlds_participants.json", "r+", encoding="utf-8"
    ) as f:
        rules_worlds_participants = json.load(f)
    with open(
        f"data/{time_path}/rules/champion_counts.json", "r+", encoding="utf-8"
    ) as f:
    ...
    new_rules = {
        ...
        | rules_countries["add"]
        | rules_finalists["add"]
        | rules_worlds_participants["add"]
        | rules_champions["add"]
        | rules_pentakills["add"]
        | rules_league_winners["add"],
        "evo": rules_teams["evo"]
        | rules_teammates["evo"]
        | rules_roles["evo"]
        | rules_countries["evo"]
        | rules_finalists["evo"]
        | rules_worlds_participants["evo"]
        | rules_champions["evo"]
        | rules_pentakills["evo"]
        | rules_league_winners["evo"],
        "rem": rules_teams["rem"]
        + rules_teammates["rem"]
        + rules_roles["rem"]
        + rules_countries["rem"]
        + rules_finalists["rem"]
        + rules_worlds_participants["rem"]
        + rules_champions["rem"]
        + rules_pentakills["rem"]
        + rules_league_winners["rem"],
    }
    ...
    with open(
        f"data/{time_path}/scripts/update_players.sql", "w+", encoding="utf-8"
    ) as f:
        f.write(plr_sql)
    with open(
        f"data/{time_path}/scripts/update_players_restore.sql", "w+", encoding="utf-8"
    ) as f:
        f.write(plr_sql_rem)
    ## Teams
    teams_sql, teams_sql_undo = generate_teams_sql_new(new_teams, old_teams)
    with open(
        f"data/{time_path}/scripts/update_teams.sql", "w+", encoding="utf-8"
    ) as f:
        f.write(teams_sql)
    with open(
        f"data/{time_path}/scripts/update_teams_restore.sql", "w+", encoding="utf-8"
    ) as f:
        f.write(teams_sql_undo)
    ## Rules
    rules_sql, rules_sql_undo = generate_rules_sql(new_rules, old_rules)
    with open(
        f"data/{time_path}/scripts/update_rules.sql", "w+", encoding="utf-8"
    ) as f:
        f.write(rules_sql)
    with open(
        f"data/{time_path}/scripts/update_rules_restore.sql", "w+", encoding="utf-8"
    ) as f:
