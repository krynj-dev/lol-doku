...
def get_team_key(team_dict, term):
    return next(
        (
            k
            for i, k in enumerate(team_dict)
            if term.lower() == team_dict[k]["op"].lower()
            or term.lower() == team_dict[k]["name"].lower()
        ),
        None,
    )
...
def get_has_child_set(teams_sets: dict):
    ...
    for k in teams_sets.keys():
        if (
            teams_sets[k]["came_from"] is not None
            and get_team_key(teams_sets, teams_sets[k]["came_from"]) is not None
        ):
...
def add_all_teams(teams_sets: dict, raw_teams: list):
    for team in raw_teams:
        ...
        if overview_page not in teams_sets:
            ...
            teams_sets[overview_page]["other_names"] = teams_sets[overview_page][
                "other_names"
            ].union({overview_page, key})
    return teams_sets
...
def populate_renames(teams_sets: dict, raw_teams_renames: list):
    for rename in raw_teams_renames:
        ...
        if (
            rename["Verb"] is None
            or rename["Verb"] == "acquire"
            or old_name.lower() == new_name.lower()
        ):
            continue
        ...
        if team_name is not None and team_sisters is not None:
            ...
            if team_key is not None:
                ...
                for sister in team_sisters:
                    ...
                    if (
                        sister_key is not None
                        and sister_key.lower() != team_key.lower()
                    ):
                        sister_to_add.add(sister_key)
                    elif (
                        sister is not None
                        and sister.lower() != team_key.lower()
                        and sister.lower() != team_name.lower()
                    ):
                        sister_to_add.add(sister)
                teams_sets[team_key]["sister_teams"] = teams_sets[team_key][
                    "sister_teams"
                ].union(sister_to_add)
    return teams_sets
    ...
    for key in teams_sets.keys():
        ...
        if teams_sets[key]["came_from"] is None:  # No children
            ...
            while teams_sets[cur_key]["becomes"] is not None:  # Has parent, add data
                ...
                teams_sets[nxt_key]["other_names"] = teams_sets[nxt_key][
                    "other_names"
                ].union(teams_sets[cur_key]["other_names"])
                seen.append(cur_key)
                cur_key = nxt_key
                visited.add(cur_key)
                if cur_key in seen:
        ...
        if teams_sets[key]["came_from"] is None:  # No children
            ...
            for sis in teams_sets[cur_key]["sister_teams"]:
                ...
                if sis_key is not None:
                    teams_sets[sis_key]["other_names"] = teams_sets[sis_key][
                        "other_names"
                    ].union(teams_sets[cur_key]["other_names"])
            while teams_sets[cur_key]["becomes"] is not None:  # Has parent, add data
                nxt_key = teams_sets[cur_key]["becomes"]
...
def get_formatted_roster_data(teams_sets: dict, raw_rosters: list, level_prio: list):
    ...
    for roster in raw_rosters:
        ...
        is_premier = any(
            [x in roster["Name"] for x in ["LEC", "LPL", "LTA", "LCK", "LCP"]]
        )
        if roster_date is not None and roster_date != "":
            ...
            if team_key not in roster_last_played.keys():
                ...
                if roster_last_played[team_key][1] in level_prio and level_prio.index(
                    roster_last_played[team_key][1]
                ) > level_prio.index(roster_level):
                    roster_last_played[team_key] = (
                        roster_date,
                        roster_level,
                        is_premier,
                    )
                elif dt.datetime.strptime(
                    roster_last_played[team_key][0], "%Y-%m-%d"
                ) < dt.datetime.strptime(roster_date, "%Y-%m-%d"):
                    roster_last_played[team_key] = (
                        roster_date,
                        roster_level,
                        is_premier,
...
def delete_subordinate_sister_teams(
    teams_sets: dict, roster_recency: dict, level_prio: list
):
    never_prim = set()
    not_main = set()
    for key in teams_sets.keys():
        ...
        if len(teams_sets[key]["sister_teams"]) > 0:
            sister_group = [
                (t, roster_recency[t])
                for t in teams_sets[key]["sister_teams"].union({key})
                if t in roster_recency.keys() and roster_recency[t] != ""
            ]
            sister_group = sorted(sister_group, key=lambda x: x[0])  # Alphabetical
            sister_group = sorted(
                sister_group,
                key=lambda x: dt.datetime.strptime(x[1][0], "%Y-%m-%d"),
                reverse=True,
            )  # Most recently played
            sister_group = sorted(
                sister_group, key=lambda x: level_prio.index(x[1][1])
            )  # Highest level played
            neglected = [
                t
                for t in teams_sets[key]["sister_teams"]
                if t not in roster_recency.keys()
            ]
            master_i = 0
            if len(sister_group) == 0:
                ...
                continue
            while (
                get_team_key(teams_sets, sister_group[master_i][0]) is None
                and master_i < len(sister_group) - 1
            ):
                master_i += 1
            master_n = sister_group[master_i]
            dead_set = [
                get_team_key(teams_sets, k[0]) for k in sister_group if k != master_n
            ]
...
def delete_subset_teams(teams_sets: dict, roster_recency: dict, level_prio: list):
    ...
    for key_a in teams_sets.keys():
        for key_b in teams_sets.keys():
            if (
                key_a != key_b
                and teams_sets[key_a]["other_names"] < teams_sets[key_b]["other_names"]
                and not (key_a in key_b and "Team)" in key_b)
            ):
    ...
    for key in equiv.keys():
        equiv_group = [
            (t, roster_recency[t])
            for t in equiv[key].union({key})
            if t in roster_recency.keys()
        ]
        ...
        equiv_group = sorted(
            equiv_group, key=lambda x: level_prio.index(x[1][1])
        )  # Highest level played
        if len(equiv_group) == 0:
            not_main = not_main.union(
                [
                    k
                    for k in equiv[key].union({key})
                    if get_team_key(teams_sets, k) is not None
                ]
            )
            continue
        master_i = 0
        while (
            get_team_key(teams_sets, equiv_group[master_i][0]) is None
            and master_i < len(equiv_group) - 1
        ):
            master_i += 1
        master_n = equiv_group[master_i][0]
        dead_set = [
            get_team_key(teams_sets, k)
            for k in equiv[key].union({key})
            if k != master_n
        ]
        not_main = not_main.union(set([t for t in dead_set if t is not None]))
...
def bubble_sisters_to_parents_then_delete_child(
    teams_sets: dict, roster_recency: dict, level_prio: list, has_child_set: set()
):
    bubbled = set()
    bottoms = set()
    for k in has_child_set:
        ...
        bubbled.add(cur_key)
        while (
            teams_sets[cur_key]["came_from"] is not None
            and get_team_key(teams_sets, teams_sets[cur_key]["came_from"]) is not None
            and teams_sets[cur_key]["came_from"] not in seen
        ):
            seen.add(cur_key)
            bubbled.add(cur_key)
            ...
            bubbled.add(cur_key)
        loop_rosters = [
            (ck, roster_recency[ck]) for ck in seen | {cur_key} if ck in roster_recency
        ]
        master_n = None
...
def delete_quirky_teams(
    teams_sets: dict, roster_recency: dict, raw_teams_redirects: list
):
    # Check for teams with parents
    todel = set()
    for k in teams_sets.keys():
        if (
            teams_sets[k]["becomes"] is not None
            and teams_sets[k]["becomes"] != teams_sets[k]["came_from"]
        ):
            goto = None
            for k2 in teams_sets.keys():
                if (
                    k != k2
                    and teams_sets[k]["becomes"] in teams_sets[k2]["other_names"]
                ):
                    if goto is not None:
    ...
    for redir in raw_teams_redirects:
        ...
        if a_name in teams_sets.keys():
            for k in teams_sets.keys():
                if (
                    k != a_name
                    and p_name in teams_sets[k]["other_names"]
                    and "Team)" not in k
                ):
...
def format_for_save(teams_sets: dict, roster_recency: dict):
    for k in teams_sets.keys():
        ...
        teams_sets[k]["highest_level"] = (
            "" if k not in roster_recency else roster_recency[k][1]
        )
    return teams_sets
...
def resolve_renames(teams_sets: dict):
    renamed_keys = {
        k: (v["renamed_to"], get_team_key(teams_sets, v["renamed_to"]))
        for k, v in teams_sets.items()
        if v["renamed_to"] not in ["", None]
    }
...
def cook_teams_data(
    raw_teams: list,
    raw_teams_sister: list,
    raw_teams_renames: list,
    raw_teams_redirects: list,
    raw_rosters: list,
    write=True,
    write_loc=None,
):
    ...
    teams_sets = bubble_sisters_to_parents_then_delete_child(
        teams_sets, roster_recency, level_prio, has_child_set
    )
