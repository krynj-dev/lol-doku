...
login = "/meta/login"
token_res = requests.post(base_url+login, json={
    "username": un,
    "password": pw
})
...
for i in range(days):
    ...
    if len(day_response.data) > 0:
        continue
    body = {"date": str(day), "puzzle": base_url+puzzles.format(puzzle_ids[(i+offset) % len(puzzle_ids)])}
    insert_response = requests.post(base_url+game_daily, json=body, headers={
