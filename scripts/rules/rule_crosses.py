...
def populate_crosses(type_dicts: list):
    ...
    for key_cur in list(all_together.keys()):
        key_no += 1
        print(
            f"\rRule {key_no}/{len(all_together.keys())}", sep=" ", end="", flush=True
        )
        for key_other in list(all_together.keys()):
            if key_cur == key_other:
                continue
            crossed_players = set(all_together[key_cur]["valid_players"]).intersection(
                set(all_together[key_other]["valid_players"])
            )
