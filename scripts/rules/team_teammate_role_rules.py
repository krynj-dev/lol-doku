...
def create_team_teammate_role_rules(cooked_teams: dict, cooked_players: dict, raw_rosters: list, write=True):
    ...
    for roster in raw_rosters:
        ...
        for i in range(len(roster["RosterLinks"])):
            ...
            for role in roster["Roles"][i].split(','):
                ...
                if team_key is not None and cooked_teams[team_key]["highest_level"] == "Primary":
                ...
                if "" in player_regions:
                    print("wth")
                if player_key not in player_levels or (index_or_large(levels, player_levels[player_key]) >= index_or_large(levels, roster["TournamentLevel"])):
    ...
    teammate_rules = {k:v for k,v in teammate_rules.items() if player_levels[k] == 'Primary'}
    if write:
...
def process_rosters_mp(cooked_teams: dict, cooked_players: dict, raw_rosters: list, write=True):
    ...
    for roster in raw_rosters:
        ...
        for i in range(len(roster["RosterLinks"])):
            ...
            for role in roster["Roles"][i].split(','):
                ...
                teammate_rules[player_key]["valid_players"] |= set(
                    [this_pk_cached(cooked_players, roster["RosterLinks"][i]) for i in range(len(roster["RosterLinks"])) if roster["Roles"][i] in player_roles and player_name != roster["RosterLinks"][i] and this_pk_cached(cooked_players, roster["RosterLinks"][i]) is not None]
                    )
                teammate_rules[player_key]["regions"] |= player_regions
...
def  create_team_teammate_role_rules_mp(cooked_teams: dict, cooked_players: dict, raw_rosters: list, write=True):
    manager = Manager()
    ...
    with Pool(cpu_count() // 2, initializer=init_shared_objects, initargs=(pk_cache, pk_cache_lock, plr_cache)) as pool:
