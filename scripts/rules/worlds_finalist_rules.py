...
def create_worlds_finalist_rules(
    cooked_players: dict, raw_tournament_results: list, write=True
):
    ...
    for team_result in raw_tournament_results:
        ...
        if (
            "Worlds" in event_name
            and "Qualif" not in event_name
            and team_result["Place"] in ["1", "2"]
        ):
            ...
            p_keys = [
                get_player_key_cached(cooked_players, p, pk_cache) for p in players
            ]
            worlds_finalist_rules[rule_key]["valid_players"] |= set(
                [x for x in p_keys if x is not None]
            )
    if write:
        finalist_loc = write_to_json_file(
            "data/rules", "finalists", worlds_finalist_rules, format=False
        )
...
def create_worlds_participant_rules(
    cooked_players: dict, raw_tournament_results: list, write=True
):
    pk_cache = {}
    top_16 = set()
    ...
    for team_result in raw_tournament_results:
        ...
        if (
            "Worlds" in event_name
            and "Qualifier" not in event_name
            and team_result["Place"] in top_16
        ):
            ...
            worlds_participant_rules[rule_key]["valid_players"] |= set(
                [x for x in p_keys if x is not None]
            )
    if write:
        participant_loc = write_to_json_file(
            "data/rules", "worlds_participants", worlds_participant_rules, format=False
        )
...
def create_league_finalist_rules(
    cooked_players: dict, raw_tournament_results: list, write=True
):
    pk_cache = {}
    rules = {}
    ...
    for team_result in raw_tournament_results:
        ...
        if team_result["Region"] == "International":
            ...
            rule_key = f"{event_name} Participant"
            if (
                team_result["Place Number"] != ""
                and int(team_result["Place Number"]) <= 2
            ):
                ...
                rules[rule_key_alt]["valid_players"] |= set(
                    [x for x in p_keys if x is not None]
                )
        else:
    ...
    if write:
        loc = write_to_json_file(
            "data/rules", "tournament_results", rules, format=False
        )
