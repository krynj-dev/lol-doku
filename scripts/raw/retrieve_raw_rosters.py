...
def get_rosters(
    site: EsportsClient, write=True, write_loc=None, date=None, levels=["Primary"]
):
    where_clause = "T.TournamentLevel IN ({})".format(
    ...
    if write:
        ...
        loc = write_to_json_file(
            ...
            delimit=True,
            list_delimiter=";;",
            list_fields=["RosterLinks", "Roles", "Flags"],
        )
...
def get_roster_recency(
    site: EsportsClient, write=False, write_loc=None, date=None, levels=["Primary"]
):
    responses = read_all_from_table(
        site=site,
        tables="TournamentRosters=TR, Tournaments=T",
