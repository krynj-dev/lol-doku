...
def fetch_raw(site: EsportsClient):
    ...
    champ_players = get_players_champs(
...
def get_new_rosters(all_rosters):
    ...
    return [
        ...
        if r["Date"] == ""
        or dt.datetime.strptime(r["Date"], "%Y-%m-%d") >= old_max_date
    ]
...
def compare_teams(new_teams):
    ...
    for k in old_keys & new_keys:
        ...
        if (
            old_team["becomes"] != new_team["becomes"]
            or old_team["came_from"] != new_team["came_from"]
            or old_team["highest_level"] != new_team["highest_level"]
            or old_team["image"] != new_team["image"]
            or old_team["name"] != new_team["name"]
            or old_team["op"] != new_team["op"]
            or set(old_team["other_names"]) != set(new_team["other_names"])
            or old_team["region"] != new_team["region"]
            or old_team["short"] != new_team["short"]
            or set(old_team["sister_teams"]) != set(new_team["sister_teams"])
        ):
...
def compare_players(new_players):
    ...
    for k in removed_keys:
        for l in added_keys:
            if (
                get_player_key(new_players, k) == get_player_key(new_players, l)
                or re.search(f"{k} \(.*\)", l) is not None
            ):
    ...
    for k in old_keys & new_keys:
        ...
        if (
            old_player["age"] != new_player["age"]
            or old_player["country"] != new_player["country"]
            or old_player["display_name"] != new_player["display_name"]
            or old_player["image"] != new_player["image"]
            or old_player["name"] != new_player["name"]
            or old_player["residency"] != new_player["residency"]
            or set(old_player["alternate_names"]) != set(new_player["alternate_names"])
        ):
...
def compare_rules(source, new_rules):
    ...
    for k in old_keys & new_keys:
        ...
        if (
            set(old_rule["exclusive_crosses"]) != set(new_rule["exclusive_crosses"])
            or len(set(new_rule["regions"]) - set(old_rule["regions"])) > 0
            or len(set(new_rule["valid_players"]) - set(old_rule["valid_players"])) > 0
        ):
            changed.append((old_rule, new_rule))
            continue

    return {"add": [new_rules[k] for k in added_keys], "modified": changed}


def create_update_file(edit_obj, data_original, data_new):
    update_file = {"add": {}, "remove": {}, "evolve": {}}
...
def create_supabase_player_update(edits):
    ...
    inserts = {"players_player": [], "players_playeralternatename": {}}
    # create updates
    updates = {"players_player": {}, "players_playeralternatename": []}
    ...
    for plr in edits["add"]:
        res: APIResponse = (
            supabase.table("players_player")
            .select("id")
            .eq("display_name", plr["display_name"])
            .execute()
        )
        if len(res.data) == 0:
            continue
...
def get_new_data(site: EsportsClient):
    ...
    teams_cooked = cook_teams_data(
        teams, sister_teams, team_renames, team_redirects, rosters, write=False
    )
    # Step 3: Create the new rules
    revamp_rosters = get_new_rosters(rosters)

    team_rules, teammate_rules, role_rules = create_team_teammate_role_rules(
        teams_cooked, players_cooked, revamp_rosters, write=False
    )
    ...
    finalists_rules = create_worlds_finalist_rules(
        players_cooked, tournament_results, write=False
    )
    worlds_participant_rules = create_worlds_participant_rules(
        players_cooked, tournament_results, write=False
    )
    # Step 4: Compare
    ...
    supabase: Client = create_client(url, key)
    responses = []
    for i, (p_id, plr_body) in enumerate(
        plr_updates["updates"]["players_player"].items()
    ):
        plr_res: APIResponse = (
            supabase.table("players_player").update(plr_body).eq("id", p_id).execute()
        )
    ...
    for plr_body in plr_updates["inserts"]["players_player"]:
        plr_res: APIResponse = (
            supabase.table("players_player").insert(plr_body).execute()
        )
        responses.append(plr_res)
            ...
            [
                {
                    ...
                    "player_name_id": plr_res.data["id"],
                }
                for x in plr_updates["updates"]["players_playeralternatename"][
                    plr_body["display_name"]
                ]
            ]
        )
